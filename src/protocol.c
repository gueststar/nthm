	  ...
	  // killed. Wait on the pipe's termination signal if necessary.
{
  ...
  result = NULL;
  if ((! s) ? IER(234) : (s->valid != MAGIC) ? IER(235) : 0)
	 return NULL;
  if ((pthread_mutex_lock (&(s->lock)) ? IER(236) : 0) ? (s->valid = MUGGLE(81)) : 0)
	 return NULL;
  if (s->reader ? (*err = (*err ? *err : NTHM_NOTDRN)) : 0)
	 goto a;
  if (s->yielded ? 0 : (pthread_cond_wait (&(s->termination), &(s->lock)) ? IER(237) : 0) ? (s->valid = MUGGLE(82)) : 0)
	 goto a;
  result = s->result;
 ...
 a: if (s->yielded ? 0 : (s->yielded = 1))
	 IER(238);
  if (pthread_mutex_unlock (&(s->lock)) ? IER(239) : 0)
	 s->valid = MUGGLE(83);
	  ...
	  // waiting, hence the loop.
{
  ...
  if ((! s) ? IER(240) : (s->valid != MAGIC) ? IER(241) : s->reader ? 0 : IER(242))
	 return NULL;
  if (_nthm_drained_by (s, d = _nthm_current_context (), err) ? 0 : (*err = (*err ? *err : NTHM_NOTDRN)))
	 return NULL;
  if (((! d) ? IER(243) : (d->valid != MAGIC) ? IER(244) : 0) ? (s->valid = MUGGLE(84)) : 0)
	 return NULL;
  if ((pthread_mutex_lock (&(d->lock)) ? IER(245) : 0) ? (d->valid = MUGGLE(85)) : 0)
	 return NULL;
  while (! (done = (s->yielded ? 1 : d->killed)))
	 if (pthread_cond_wait (&(d->progress), &(d->lock)) ? IER(246) : 0)
		goto a;
  if (*err ? 0 : s->status ? (*err = s->status) : 0)
	 s->status = 0;
  result = (s->yielded ? s->result : NULL);
 a: if (pthread_mutex_unlock (&(d->lock)) ? IER(247) : 0)
	 d->valid = MUGGLE(86);
...
{
  if ((! s) ? IER(248) : (s->valid != MAGIC) ? IER(249) : 0)
	 return;
  s->yielded = 1;
  if (pthread_cond_signal (&(s->termination)) ? IER(250) : 0)
	 s->valid = MUGGLE(87);
  else if (s->killed ? 0 : s->status ? 0 : (s->status = *err))
	 *err = 0;
  if (pthread_mutex_unlock (&(s->lock)) ? IER(251) : 0)
	 s->valid = MUGGLE(88);
}
	  ...
	  // entry.
{
  ...
  if ((! s) ? IER(252) : (s->valid != MAGIC) ? IER(253) : s->killed ? IER(254) : 0)
	 return;
  if ((!(s->reader)) ? IER(255) : (!(d = s->reader->pipe)) ? IER(256) : (d->valid != MAGIC) ? IER(257) : 0)
	 goto a;
  if ((pthread_mutex_lock (&(d->lock)) ? IER(258) : 0) ? (d->valid = MUGGLE(89)) : 0)
	 goto a;
  if (((e = d->scope) ? 0 : IER(259)) ? (d->valid = MUGGLE(90)) : 0)
	 goto b;
  if ((((l = _nthm_scope_level (d, err)) < s->depth) ? IER(260) : 0) ? (s->valid = MUGGLE(91)) : 0)
	 goto b;
  for (l = l - s->depth; l; l--)
	 if (((e = e->enclosure) ? 0 : IER(261)) ? (d->valid = MUGGLE(92)) : 0)
		goto b;
  ...
  if ((s->yielded ? 0 : _nthm_freed (b, err) ? 1 : IER(262)) ? (s->yielded = 1) : 0)
	 s->valid = MUGGLE(93);
  if (pthread_cond_signal (&(d->progress)) ? IER(263) : 0)
	 d->valid = MUGGLE(94);
  if (s->status ? 0 : (s->status = *err))
	 *err = 0;
 b: if (pthread_mutex_unlock (&(d->lock)) ? IER(264) : 0)
	 d->valid = MUGGLE(95);
 a: if (pthread_mutex_unlock (&(s->lock)) ? IER(265) : 0)
	 s->valid = MUGGLE(96);
}
...
{
  if ((! _nthm_descendants_killed (source, err)) ? 1 : (! source) ? IER(266) : (source->valid != MAGIC) ? IER(267) : 0)
	 return;
  if ((pthread_mutex_lock (&(source->lock)) ? IER(268) : 0) ? (source->valid = MUGGLE(97)) : 0)
	 return;
	  ...
	  // finished.
{
  ...
  err = 0;
  if ((t = (thread_spec) void_pointer) ? 0 : (deadlocked = err = THE_IER(269)))
	 goto a;
  if (((!(s = t->pipe)) ? 1 : (s->valid != MAGIC) ? 1 : ! _nthm_set_context (s, &err)) ? (deadlocked = err = THE_IER(270)) : 0)
	 goto b;
